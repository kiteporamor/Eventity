...
for i in range(10000):
    pg_cursor.execute("INSERT INTO events (name, data) VALUES (%s, %s)", (f"Event {i}", {"value": i}))
pg_conn.commit()
pg_conn.close()

# MongoDB connection
mongo_client = pymongo.MongoClient("mongodb://mongo-bench:27017")
mongo_db = mongo_client["EventityBench"]
mongo_collection = mongo_db["events"]
mongo_collection.drop()  # Reset
for i in range(10000):
    mongo_collection.insert_one({"name": f"Event {i}", "data": {"value": i}})
mongo_client.close()

print("Data generated for both DBs")